...
export function StatTrapTemplate({ context }: BlankTemplateVariantProps) {
  const { tr, headerText, subText, blockFields } = context
  const trapTop =
    pickTemplateField(blockFields, ['trap_top', 'top', 'line_1']) || tr('REGEN I BRUTALNOSC >', 'REGEN AND BRUTALITY >')
  const trapBottom =
  ...
  const example =
    ...
    tr(
      'Roznica 2-3 punktow w umiejetnosciach zanika, gdy przeciwnik leczy sie natychmiastowo po kazdym ciosie.',
  ...
  const questionLine =
    ...
    tr(
      "KLUCZOWE PYTANIE: W trybie 'Kill-Only' regeneracja przeciwnika wazy wiecej niz statystyki all-around.",
